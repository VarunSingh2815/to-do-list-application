...
def update_task():
    selected_task = task_listbox.get(tk.ACTIVE)  # Get the selected task from the listbox
    new_task_text = update_field.get()  # Get the updated text from the entry field

    if not selected_task:
        messagebox.showinfo('Error', 'No Task Selected. Cannot Update.')
    elif not new_task_text:
        messagebox.showinfo('Error', 'Field is Empty. Cannot Update.')
    else:
        index = tasks.index(selected_task)
        tasks[index] = new_task_text  # Update the task in the list
        task_listbox.delete(tk.ACTIVE)  # Delete the old task from the listbox
        task_listbox.insert(tk.ACTIVE, new_task_text)  # Insert the updated task
        update_field.delete(0, 'end')  # Clear the entry field
...
if __name__ == "__main__":
    ...
    header_frame = tk.Frame(guiWindow, bg="#8DEEEE")
    functions_frame = tk.Frame(guiWindow, bg="#8DEEEE")
    listbox_frame = tk.Frame(guiWindow, bg="#8DEEEE")
    ...
    update_button.place(x=30, y=280)
    update_field = ttk.Entry(
        functions_frame,
        font=("Consolas", "12"),
        width=16,
        background="#FFF8DC",
        foreground="#A52A2A")
    update_field.place(x=30, y=320)
    ...
    task_listbox.place(x=00, y=0)
    # Update the list with tasks and due dates
    list_update()
